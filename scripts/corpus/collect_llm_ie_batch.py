...
Migration: --state also accepts batch_state_onepass.json from prior runs.
"""
...
from raqr.data.llm_onepass import _post_process_onepass, parse_onepass_batch_output_line
...
def main() -> int:
    ...
    parser.add_argument(
        "--state",
        required=True,
        help="Path to batch_state_ie.json (or batch_state_onepass.json) from submit script.",
    )
    parser.add_argument(
        "--corpus",
        default=None,
    ...
    output_path = Path(args.output) if args.output else output_dir / DEFAULT_OUTPUT_FILENAME
    ...
    for shard in shards:
        ...
        for line in text.strip().split("\n"):
            ...
            if obj.get("error"):
                ...
                continue
            completed += 1
            cid, raw_entities, raw_triples = parse_onepass_batch_output_line(obj)
