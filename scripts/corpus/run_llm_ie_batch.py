...
Migration: checks batch_state_ie.json first, then batch_state_onepass.json.
Checks corpus_llm_ie.jsonl first, then corpus_llm_onepass.jsonl.
...
STATE_FILENAMES = ("batch_state_ie.json", "batch_state_onepass.json")
OUTPUT_FILENAMES = ("corpus_llm_ie.jsonl", "corpus_llm_onepass.jsonl")
...
def _wait_for_batch(client: OpenAI, batch_id: str, poll_seconds: int, timeout_seconds: int) -> bool:
    ...
    while True:
        ...
        logger.info("Batch %s status=%s (elapsed %.1f min). Polling in %d s...", batch_id, status, elapsed / 60, next_poll)
...
def main() -> int:
    ...
    if not wiki_titles_path.is_file():
        ...
        if _run_script("build_wiki_title_seed.py", "--corpus", str(corpus_path), "--output-dir", str(output_dir)) != 0:
    ...
    if not state_path or not state_path.is_file():
        ...
        if _run_script(SUBMIT_SCRIPT, "--corpus", str(corpus_path), "--output-dir", str(output_dir)) != 0:
    ...
    if not args.no_wait:
        ...
        for shard in shards:
            ...
            if b.status != "completed":
                if not _wait_for_batch(client, batch_id, args.poll_seconds, args.timeout_seconds):
                    all_ok = False
        if not all_ok:
