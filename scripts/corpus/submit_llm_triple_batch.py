"""Submit an OpenAI Batch API job for LLM triple extraction from corpus chunks.

Usage:
    poetry run python scripts/corpus/submit_llm_triple_batch.py --corpus data/processed/corpus.jsonl
    poetry run python scripts/corpus/submit_llm_triple_batch.py --corpus data/processed/corpus.jsonl --limit 10

Reads corpus.jsonl, builds batch request JSONL, uploads to OpenAI, creates a batch job.
Saves batch_state.json so you can collect results later. You can close your computer
after submission; the job runs server-side.
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Submit OpenAI Batch API job for LLM triple extraction.",
    )
    parser.add_argument(
        "--corpus",
        required=True,
    ...
    parser.add_argument(
        ...
        help="Directory for batch_input.jsonl, batch_state.json, batch_manifest.jsonl.",
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    prompt_template = get_triple_extractor_prompt()
    ...
    if count > BATCH_LIMIT_REQUESTS:
        logger.warning(
            "Batch exceeds %d requests (%d). Consider splitting.",
            BATCH_LIMIT_REQUESTS,
            count,
        )
    if total_bytes > BATCH_LIMIT_BYTES:
        logger.error(
            "Batch input file exceeds 200MB (%.1f MB). Split into smaller batches.",
            total_bytes / (1024 * 1024),
        )
        return 1

    logger.info("Created batch_input.jsonl with %d requests (%.1f KB)", count, total_bytes / 1024)
    ...
    logger.info("Creating batch job (completion_window=%s)...", args.completion_window)
    batch = client.batches.create(
        input_file_id=uploaded.id,
        endpoint="/v1/chat/completions",
        completion_window=args.completion_window,
        metadata={"description": "LLM triple extraction", "corpus": str(corpus_path)},
    )
