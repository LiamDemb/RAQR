...
def main() -> int:
    ...
    parser.add_argument("--limit", type=int, default=None, help="Max chunks to include.")
    parser.add_argument(
        "--completion-window",
        default="24h",
    ...
    if not wiki_titles_path.is_file():
        logger.error("wiki_titles.jsonl not found at %s. Run build_wiki_title_seed first.", wiki_titles_path)
        return 1
    ...
    def _flush_shard(batch_out):
        ...
        batch = client.batches.create(
            ...
            metadata={"description": "LLM IE extraction", "corpus": str(corpus_path), "shard": str(shard_idx)},
        )
        shards.append({
            "batch_id": batch.id,
    ...
    try:
        for chunk in chunks:
            ...
            seed_titles = matcher.find_titles_in_text(text_for_extraction, max_results=50)
            seed_str = "\n".join(f"- {s}" for s in seed_titles) if seed_titles else "(none detected)"
            ...
            body = build_onepass_chat_request(prompt)
            batch_line = build_onepass_batch_line(custom_id=chunk_id, body=body)
    ...
    logger.info("Submitted %d shard(s), %d total requests. State: %s", len(shards), state["total_requests"], state_path)
    for s in shards:
        print(s["batch_id"])
    return 0
