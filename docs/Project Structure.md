...
- **`data/`**: Logic for loading, cleaning, and normalizing datasets (NQ, 2WikiMultiHopQA, etc.). Contains the unified schema definitions.
...
- **`strategies/`**: The 3 implementations of retrieval (Dense, Graph). Each must adhere to a common interface and return a structured result (e.g., `StrategyResult`: `answer`, `contexts`, `scores`, `latency_ms`) for consistent evaluation and error analysis.
...
_Note: These folders are `.gitignored`._
