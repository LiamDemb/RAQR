...
- Compute \(\Delta = S*{best} - S*{second_best}\) for each Dev example (using LLM-as-judge scores).
...
- **Entity density:** Number of entities per token (from query entity extraction); higher density may indicate multi-hop or graph-relevant queries.
...
_Derived from a "Probe Search" (a fast top-10 Dense retrieval using the Dense index)._
...
- **Mechanism:** Supervised classification.
    - _Text Input:_ Tokens passed through Transformer layers.
    - _Signal Input:_ Numerical features concatenated to the classification head (MLP).
    - _Head:_ Layer 2: Linear (256 -> 3) (3 Classes).
