...
**Important:** The pipeline is designed to be fully **incremental**. It "fills in the gaps." If you process 1,000 questions and later decide to add another 2,000, running the pipeline commands again will _not_ overwrite your existing data or re-evaluate already-answered questions. It will only process the novel data and merge the new results, saving significant time and OpenAI API costs.
...
- **Incremental:** It looks for an existing `benchmark.jsonl` and dedupes incoming data by `question_id`. Only novel questions are appended. Train/dev/test splitting for the router is done later in `make build-router-dataset`.
...
- **Incremental:** The script reads any existing `oracle_raw_scores.jsonl`. If a question has already been answered by both strategies, it is excluded from the new batch. After the batch job finishes, results are **merged** into `oracle_raw_scores.jsonl` (existing rows are preserved; new predictions are filled in). Stratified train/dev/test splits for training are **not** on the benchmark; they are applied in step 4.
- **Collect only (no new batch):** If a batch already completed but results were not merged (e.g. interrupted run), run `make collect-answer-batch` (uses `data/processed/batch_state_strategy.json` by default). This downloads outputs and merges into `oracle_raw_scores.jsonl` without submitting another batch.
...
make build-router-dataset-undersample
```

_(Note for batch commands: Ensure you wait for the OpenAI batches to complete. The `run\__`commands orchestrate both submit and collect. Use`make collect-answer-batch` if you only need to download and merge completed batch output.)\*
