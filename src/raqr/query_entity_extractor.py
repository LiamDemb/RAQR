"""LLM-based query entity extractor for GraphStrategy.

Replaces regex/spaCy with a fast LLM call. Returns normalized entity strings
for graph node matching.
"""
...
class LLMQueryEntityExtractor:
    ...
    def extract(self, query: str) -> List[str]:
        ...
        from raqr.prompts import get_query_entity_extraction_prompt
        ...
        for surface in raw_entities:
            if not surface:
                continue
            norm = self.alias_resolver.normalize(surface) if self.alias_resolver else normalize_key(surface)
