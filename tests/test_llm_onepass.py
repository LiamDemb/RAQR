...
from raqr.data.llm_onepass import (
    _find_evidence_span,
    _post_process_onepass,
)
...
def test_post_process_dedupes_entities_by_norm() -> None:
    ...
    raw_triples = []
    entities, _ = _post_process_onepass(raw_entities, raw_triples, "text", None)
...
def test_post_process_filters_banned_predicates() -> None:
    ...
    _, relations = _post_process_onepass(raw_entities, raw_triples, "Eva Busch was a singer.", None)
...
def test_post_process_injects_endpoint_entities() -> None:
    ...
    entities, relations = _post_process_onepass(raw_entities, raw_triples, "text", None)
...
def test_post_process_shape() -> None:
    ...
    entities, relations = _post_process_onepass(
    ...
    assert rec["rule_id"] == "LLM_ONEPASS_V1"
...
def test_post_process_skips_incomplete_triple() -> None:
    ...
    _, relations = _post_process_onepass([], raw_triples, "text", None)
...
def test_post_process_dedupes_triples_by_norm_key() -> None:
    ...
    _, relations = _post_process_onepass(raw_entities, raw_triples, "text", None)
    assert len(relations) == 1
